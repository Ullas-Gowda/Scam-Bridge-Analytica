...
# Debug info for environment troubleshooting
...
print(f"GEMINI_API_KEY present in environment: {bool(GEMINI_KEY)}")
...
def call_gemini_chat(prompt: str) -> str:
    ...
    try:
        ...
        if hasattr(genai, "chat") and hasattr(genai.chat, "create"):
            ...
            resp = genai.chat.create(model="models/gemini-pro-latest", messages=messages)
...
def display_prediction(prediction, confidence):
    ...
    if prediction == 'positive':
        st.success(f"😊 Predicted Sentiment: POSITIVE (Confidence: {confidence:.2f})")
    elif prediction == 'negative':
        st.error(f"❌ Predicted Sentiment: NEGATIVE (Confidence: {confidence:.2f})")
    elif prediction == 'neutral':
        st.warning(f"😐 Predicted Sentiment: NEUTRAL (Confidence: {confidence:.2f})")
    else:
        st.info(f"❓ Predicted Label: {prediction} (Confidence: {confidence:.2f})")


# ==============================================================
# 1. STREAMLIT CONFIGURATION
# ==============================================================
st.title("🤡.Scam-Bridge Analytica")
st.markdown("---")
...
user_input = st.text_area("✍️ Enter Tweet Text:", height=100, placeholder="Example: The new policy proposal is the worst decision this year.")
...
if uploaded_file is not None:
    ...
    if st.button("🚀 Run Bulk Analysis", key="run_bulk", use_container_width=True):
...
st.markdown("Ask general political questions or current event summaries — powered by **Google Gemini**.")

# Sidebar Debug Info (for easy troubleshooting)
with st.sidebar.expander("🔧 Gemini Debug Info"):
    st.write({
...
st.info(f"🔧 Debug: SDK available: {HAVE_GENAI}, API key found: {bool(GEMINI_KEY)}, Chat enabled: {CHAT_ENABLED}")


# Handle Chat UI
if not CHAT_ENABLED:
    st.error("🚫 Gemini Chatbot is disabled.")
    ...
    # Input from user
    if prompt := st.chat_input("💬 Ask a political question..."):
        # Show user message
        ...
        with st.chat_message("assistant"):
            with st.spinner("🤔 Thinking politically..."):
